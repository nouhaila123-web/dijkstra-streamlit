...
if page == "Accueil":
    ...
    **D(i) ← min( D(i), D(j) + l(j, i) )**
    ...
    n = st.number_input("Nombre de sommets", min_value=2, max_value=10, value=4)
    ...
    mode = st.radio(
        "Mode de calcul",
        ("Un seul sommet d’arrivée", "Tous les sommets"),
        key="mode_calcul"
    )
    ...
    if mode == "Un seul sommet d’arrivée":
        ...
        st.info("Les distances seront calculées vers tous les sommets.")
    ...
    def dijkstra(g, start):
        ...
        while len(visited) < len(g):
            u = min(
                (n for n in g if n not in visited),
                key=lambda x: D[x]
            )
            visited.add(u)
    ...
    if st.button("Calculer le plus court chemin"):
        ...
        if mode == "Un seul sommet d’arrivée":
            ...
            st.success(
                ...
                f"| Distance = {distances[target]}"
            )

            st.subheader("📈 Graphe avec plus court chemin")
            ...
            edge_colors = [
                "red" if (u, v) in shortest_edges else "gray"
                for u, v in G.edges()
            ]
            edge_widths = [
                3 if (u, v) in shortest_edges else 1
                for u, v in G.edges()
            ]
        ...
        # ===== MODE 2 : TOUS LES SOMMETS =====
        else:
            st.subheader("📊 Distances depuis le sommet de départ")

            result = {
                node: distances[node] if distances[node] < math.inf else "∞"
                for node in nodes
            }
            ...
            st.table(
                pd.DataFrame.from_dict(
                    result,
                    orient="index",
                    columns=["Distance minimale"]
                )
            )

            st.subheader("📈 Graphe – Arbre des plus courts chemins")
